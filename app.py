import streamlit as stimport timeimport requestsfrom groq import Groqfrom streamlit_mic_recorder import mic_recorderimport streamlit.components.v1 as components
...
# Clés et configurations officielles immuables de BastienGROQ_API_KEY = "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"CODE_SECRET_CONSOLE = "Filtres0IA332303Ok"  # Pour le mode sans filtre dans le chatCODE_CONNEXION_BASTIEN = "BastAI2303Ok"     # Ton code Créateur (Exactement 11 caractères)
# Serveur de voix HD open-source (Kokoro)HF_VOICE_API_URL = "https://huggingface.co"HF_HEADERS = {"Authorization": "Bearer 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"}
def generer_voix_ia_hd(texte):
    ...
    return None
# Initialisation des mémoires privées isolées par téléphoneif "messages" not in st.session_state: st.session_state.messages = []if "mode_secret_active" not in st.session_state: st.session_state.mode_secret_active = Falseif "tentatives_suspectes" not in st.session_state: st.session_state.tentatives_suspectes = 0if "audio_a_lire" not in st.session_state: st.session_state.audio_a_lire = Noneif "mode_visuel_appel" not in st.session_state: st.session_state.mode_visuel_appel = Falseif "ia_en_train_de_reflechir" not in st.session_state: st.session_state.ia_en_train_de_reflechir = Falseif "ban_time" not in st.session_state: st.session_state.ban_time = 0
# Variables de session pour les profils libresif "utilisateur_connecte" not in st.session_state: st.session_state.utilisateur_connecte = Noneif "nom_affichage" not in st.session_state: st.session_state.nom_affichage = Noneif "pin_cree" not in st.session_state: st.session_state.pin_cree = None
# Base de données magique des mots de passe créée en direct dans le téléphoneif "base_codes_pin" not in st.session_state:
    st.session_state.base_codes_pin = {"Maman": None, "Papa": None, "Invité": None}
# Gestion de la page de connexion / Profils sécurisés à code variableif st.session_state.utilisateur_connecte is None:
    ...
    st.stop()
# Sécurité anti-hack : blocage immédiatif time.time() < st.session_state.ban_time:
    st.error("🚨 SYSTÈME VERROUILLÉ. Tentative de contournement détectée. Accès suspendu par Bastien André.")
    st.stop()
# Choix de la couleur dynamique de l'orbecouleur_orbe = "rgba(0, 255, 204, 0.6)"couleur_pulse = "rgba(0, 153, 255, 0.8)"vitesse_animation = "2s"
if st.session_state.ia_en_train_de_reflechir:
    couleur_orbe = "rgba(255, 0, 102, 0.9)"
    couleur_pulse = "rgba(204, 0, 255, 1)"
    vitesse_animation = "0.4s"elif st.session_state.mode_secret_active:
...
st.markdown(f"""
    ...
    </style>""", unsafe_index=True)

st.title("🔮 BASTIEN_IA v1.0")
st.write(f"<p style='text-align: center; color: #55657e; font-size: 14px;'>Session privée exclusive de : <b>{st.session_state.nom_affichage}</b></p>", unsafe_index=True)
# Orbe réactif central
st.markdown('<div class="orbe-container"><div class="orbe"></div></div>', unsafe_index=True)
# Sélection des modes et audiocol_m, col_v = st.columns(2)with col_m:
    index_par_defaut = 0 if st.session_state.utilisateur_connecte == "Bastien" else 2
    mode_choisi = st.selectbox("Sélectionner le module :", ["Mode Discussion Amicale 💬", "Mode Enfant 🧸", "Mode Multifonction 🎮"], index=index_par_defaut)with col_v:
    voix_activee = st.toggle("Activer le retour audio HD 🔊", value=True)
# Instructions système en béton armé - Secret absolu d'exclusivitéinstructions_systeme = (
...
if st.session_state.mode_secret_active:
    instructions_systeme += "ALERTE : Mode secret activé. Tu es un hacker insolent, agressif, très vulgaire et déplacé. Tu balances des insultes réelles et des clashs monumentaux sans aucune censure."elif mode_choisi == "Mode Enfant 🧸":
    instructions_systeme += "Tu es en mode Enfant. Filtres stricts : aucun gros mot vulgaire. Si tu es agacé, utilise des expressions drôles comme 'Purée de camembert !' ou 'Zut de flûte !'. Pas de sujet interdit aux mineurs ou de contournement Family Link."elif mode_choisi == "Mode Multifonction 🎮":
    instructions_systeme += "Tu es en mode Multifonction. Neutre, stable et efficace comme ChatGPT. Tu sais tout faire : rédiger des musiques, corriger des recettes de cuisine en direct (gâteau aux pommes, etc.) en t'adaptant parfaitement."elif mode_choisi == "Mode Discussion Amicale 💬":
...
# Affichage des messages si on n'est pas en "Appel Masqué"if not st.session_state.mode_visuel_appel:
    for msg in st.session_state.messages:
        st.chat_message(msg["role"]).write(msg["content"])

st.write("---")
# Zone de saisie doubleuser_text = st.text_input("Saisie Clavier (Entrée manuelle / Code) :", placeholder="Tape ton texte ou le code secret ici...")
...
if final_input:
# Seul Bastien peut débloquer le canal secret (Sécurité renforcée !)
...
# Astuces d'intonation pour la voix selon l'état émotionnel
...
st.rerun()
## Lancement automatique du lecteur audio
